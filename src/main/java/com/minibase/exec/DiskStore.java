...
import com.minibase.exec.DiskFile.DiskFileWriter;
import com.minibase.exec.MStore.SeekIter;
import com.minibase.exec.MiniBase.Compactor;
import com.minibase.exec.MiniBase.Flusher;
import com.minibase.exec.MiniBase.Iter;
    ...
    public void flush(Iter<KeyValue> it) throws IOException {
      ...
      try {
        try (DiskFileWriter writer = new DiskFileWriter(fileTempName)) {
          ...
          /**
           * 写入indexblock 保存了此次写入的所有datablock的索引数据
           * 该datablock的lastKv
           * 该datablock的lastKv在diskfile中的偏移量
           * 该datablock占用的字节长度
           * 该datablock的布隆过滤器
           */
          writer.appendIndex();
